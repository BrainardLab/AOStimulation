function [diffMax,diffMin,linePixelTimeTable,desinMovies,simpleDesinMovies] = aoRegDesin(desinMatrix,rawMovies,maxMovieLength)
...
%
% Outputs:
%    desinMovies        - desinsoided movides without simplified maxtrix.
%    simpleDesinMovies  - desinsoided movides with simplified maxtrix.
%    diffMax            - the maximum diff between using original maxtrix and
%                       - using simplified maxtrix
%    diffmin            - the maximum diff between using original maxtrix and
%                       - using simplified maxtrix
%
...
for i=1:s1
    ...
    desinMatrix1(i,selectP0Idx) = desinMatrix(i,selectP0Idx);
    desinMatrix1(i,selectP1Idx) = desinMatrix(i,selectP1Idx);
    desinMatrix1(i,selectP2Idx) = desinMatrix(i,selectP2Idx);
    desinMatrix1(i,selectP3Idx) = desinMatrix(i,selectP3Idx);
    desinMatrix1(i,selectP4Idx) = desinMatrix(i,selectP4Idx);
    desinMatrix1(i,selectP5Idx) = desinMatrix(i,selectP5Idx);   
end

%line pixel Table
for i=1:s1
    lineT = find(desinMatrix1(i,:)~=0);
    linePixelTimeTable(i)= lineT(end);
end
...
for frameIdx=1:maxMovieLength
    ...
    newDesinImage = uint8(fix(rawImage * desinMatrix1'));
    
    %generate the movies
    desinMovies(:,:,frameIdx) = oriDesinImage;
    simpleDesinMovies(:,:,frameIdx) = newDesinImage;
    
    %Get the different result by the two array
    diffMax(frameIdx) = max(max(newDesinImage-oriDesinImage));
    diffMin(frameIdx) = min(min(newDesinImage-oriDesinImage));
